...
interface CourseSidebarProps {
  ...
  getContentHref: (
    ...
    subchapter: SidebarSubchapter,
    type: "main" | "summary" | "quiz"
  ) => string;
  storageKey: string;
...
export function CourseSidebar({
  ...
  const chapterHasActivePath = (chapter: SidebarChapter) =>
    chapter.subchapters.some((subchapter) =>
      (["main", "summary", "quiz"] as const).some((type) => {
        const href = getContentHref(chapter, subchapter, type);
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
    ...
    chapters.forEach((chapter) => {
      map[chapter.id] = chapterHasActivePath(chapter);
    });
  ...
  const getDefaultExpandedState = () => {
    ...
    chapters.forEach((chapter) => {
      defaults[chapter.id] = chapterHasActivePath(chapter);
    });
  ...
  function getChapterCompletionStatus(chapter: SidebarChapter) {
    const allContentIds = chapter.subchapters.flatMap((subchapter) =>
      subchapter.contents
        .filter((content) => !content.lang || content.lang === "zh")
      ...
      <div className="border-b p-5">
        <Link href={overviewHref}>
          <h2 className="line-clamp-3 text-base font-bold leading-7 transition-colors hover:text-primary sm:text-lg">
            {lang === "en" && courseTitleEn ? courseTitleEn : courseTitle}
          </h2>
        </Link>
      </div>
